...
class VolumeLimitExceededError(Exception):
    pass
...
class Tank:
    ...
    def pour_water(self, volume: int) -> bool:
        ...
        if self.capacity - self.volume_filling < volume:
            raise VolumeLimitExceededError(f'There is not enough space in {self.name}\n'
    ...
    def pour_out_water(self, volume: int) -> bool:
        ...
        if self.volume_filling - volume < 0:
            raise VolumeLimitExceededError(f'There is not enough water in {self.name}\n'
                                           f'Please try different tank.\n')
